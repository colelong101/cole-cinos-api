from api.cinos import Drink, Order

""" Drink class tests section """
...
""" Order class test section """

def test_get_items_when_empty():
    """
    Test if get_items returns an empty array when no items are added to order
    """
    order = Order()
    assert order.get_items() == []

def test_get_num_items_when_emtpy():
    """
